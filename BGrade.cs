...
namespace Oxide.Plugins
{
    ...
    public class BGrade : RustPlugin
    {
        ...
        private void OnEntityBuilt(Planner plan, GameObject gameObject)
        {
            ...
            if (buildingBlock == null || !player.CanBuild() || !player.HasAnyPermission(_registeredPermissions)) return;
            ...
            if (!player.HasPluginPerm("all") && !player.HasPluginPerm(playerGrade.ToString())) return;
            ...
            if (playerGrade < (int)buildingBlock.grade || buildingBlock.blockDefinition.grades[playerGrade] == null) return;
            if (CheckLastAttack && WasAttackedRecently(buildingBlock.transform.position)) return;

            bool hasNoRes = player.HasPluginPerm("nores");
            if (!hasNoRes)
            {
            ...
            NextTick(() =>
            {
                ...
                buildingBlock.ChangeGradeAndSkin((BuildingGrade.Enum)playerGrade, 0, true, true);
                buildingBlock.SetHealthToMax();
                buildingBlock.StartBeingRotatable();
                buildingBlock.ResetUpkeepTime();
                buildingBlock.GetBuilding()?.Dirty();
                Interface.Call("OnStructureUpgrade", buildingBlock, player, (BuildingGrade.Enum)playerGrade, 0ul);
            });
        }

        private object OnPayForPlacement(BasePlayer player, Planner planner, Construction component)
        {
            if (planner.isTypeDeployable || !BGradePlayer.Players.ContainsKey(player)) return null;
